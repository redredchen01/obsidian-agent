...
export function bridgeGithub(vaultRoot, options = {}) {
  ...
  try {
    ...
    try {
      releasesJson = run(`gh release list --repo ${repoParam} --limit 10 --json tagName,name,createdAt,body`);
    } catch {
      releasesJson = '[]';
    }
    ...
    if (releases.length > 0) {
      ...
      const releaseText = `## 最新版本\n\n**${recentRelease.tagName}** (${new Date(recentRelease.createdAt).toISOString().split('T')[0]})\n\n${recentRelease.body || '(no changelog)'}`;
