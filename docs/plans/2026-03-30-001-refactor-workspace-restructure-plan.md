...
- 不改變有獨立 .git 的專案內部結構（gwx、claude_code_telegram_bot、ctx、obsidian-agent、session-wrap-skill）
...
- remotion-clip 是否還需要？→ 低活躍度，保留在 experimental 但不列為主專案
...
- Delete: `projects/experimental/0323/`
- Delete: `projects/experimental/APP/`
- Delete: `projects/experimental/sub2api-deploy/`
- Delete: `NS_0327` (symlink at root)
- Modify: `.gitignore` (移除不再需要的規則)

**Approach:**
- 依次刪除空目錄和廢棄目錄
- 移除根目錄 NS_0327 symlink
- 確認 .gitignore 中沒有引用這些路徑的規則

**Patterns to follow:**
- 2026-03-27 cleanup session 的做法：確認空目錄後直接刪除

**Test expectation:** none -- 純文件系統操作，無行為變化

**Verification:**
- `ls projects/production/` 只顯示 claude_code_telegram_bot 和 gwx
- `ls projects/experimental/` 只顯示 NS_0327 和 remotion-clip
- 根目錄無 NS_0327 symlink
...
- Move to archive: `docs/AGENT-WORKFLOW.md`, `docs/BENCHMARKS.md`, `docs/CONTRIBUTING.md`, `docs/DASHBOARD-DEPLOYMENT-GUIDE.md`, `docs/ENTERPRISE-ADOPTION.md`, `docs/FAQ-TROUBLESHOOTING.md`, `docs/INTEGRATIONS.md`, `docs/MIGRATION.md`, `docs/OPENSOURCE-COLLABORATION.md`, `docs/PRODUCTION-SETUP.md`, `docs/QUICKSTART-EXAMPLES.md`, `docs/SECURITY.md`, `docs/SOLO-DEVELOPER.md`, `docs/STARTUP-TEAM-WORKFLOW.md`, `docs/TEAM-RITUALS.md`, `docs/TESTING.md`, `docs/VISUALIZATION-GUIDE.md`, `docs/SESSION-WRAP-QUICKSTART.md`
...
- 更新路徑、技術棧、狀態信息
- 更新 tools 項目信息（ctx、obsidian-agent、session-wrap-skill、session-wrap-backend）
...
**Files:**
- Review: `scripts/` 中所有 28 個腳本
- Potential moves to archive: 與 session-wrap 產品相關的腳本（deploy-railway.sh、validate-demo.sh、integration-test.sh、visualize-tasks.sh、test-agent-tools.sh）
- Keep: agent-trace.sh、obsidian-*.sh、session-wrap.sh、skill-deploy.sh、archive-old-sessions.sh、trace-*.sh、setup.sh

**Approach:**
- 將純 session-wrap 產品腳本移到 `scripts/archive/`
- 保留工作區管理和 agent 工具腳本
- 確認哪些腳本被 hook 或 cron 引用，避免破壞
...
- **Interaction graph:** `.zshrc-workspace` 被 shell 啟動載入；`settings.json` hook 引用 scripts/ 中的腳本；CLAUDE.md 被 Claude Code 每次會話載入；PROJECTS_INFO.md 被 `yd-info` 引用
- **Error propagation:** 路徑錯誤會導致 alias 失效（無害）或 hook 報錯（中斷工作流）
- **State lifecycle risks:** 無持久化數據風險，純文件系統操作
- **Unchanged invariants:** 有獨立 `.git` 的專案（gwx、claude_code_telegram_bot、ctx、obsidian-agent、session-wrap-skill）內部結構不變；obsidian/ 知識庫不變；memory 文件不變
