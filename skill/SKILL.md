...
## Claude Code 深度整合（v1.1+）

新功能（Phase 2-4 實現中）：

| 功能 | 使用 | 說明 |
|------|------|------|
| Memory 同步 | `memory sync [--dry-run]` | 同步 vault 筆記到 Claude memory |
| Context 注入 | `context-for-topic "主題"` | 快速取得話題相關的 vault context（RAG）|
| CLAUDE.md 管理 | `claude-md inject --global` | 自動注入 vault context 到 CLAUDE.md |
| Hook 自動化 | `hook session-start/pre-tool-use` | 自動記錄工作活動和每日 context |

詳見計畫中的「Phase 2-4: Hooks 深化」部分。
